...
class Log {
    ...
    static async access(ctx, time) {
        ...
        request.ua = Object.assign({}, ua, { os: ua.os });
    ...
    static async error(ctx, err) {
        ...
        // e-mail notification
        try {
            const to = 'hasim.imahdudin@gmail.com'; // could be from env var
            if (ctx.response.status == 500) await Mail.sendText(to, 'Koa Sample Web App 500 error', err.stack, ctx);
        } catch (e) {
            console.error('log', e);
        }
    }
